...
class Goals
  ...
  def goal_term(savings, hash)
    ...
    puts "Using #{(array[0][:percentage])}% of your savings ($#{(savings * (array[0][:percentage].to_f / 100.to_f)).to_i.to_s.light_green}/month), in #{(array[0][:cost].to_f / (savings * (array[0][:percentage].to_f / 100.to_f))).ceil} month(s) you'll have enough for '#{goal.capitalize.to_s.light_yellow}' ($#{array[0][:cost].to_s.light_green}).".center(80, " ").rstrip
    end
    prompt = TTY::Prompt.new
